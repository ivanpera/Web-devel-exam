<form action="TODO: add php form processor" method="POST">
    <label for="email">Email: <br/><input type="email" id="email" name="email""></input></label><br/>
    <label for="password">Password: <br/><input type="password" id="password" name="password" required></input></label><br/>
    <label for="password_confirmation">Reinserisci la password: <br/><input type="password" id="password_confirmation" name="password_confirmation" required></input></label><br/>
    <label for="name">Nome e cognome: <br/><input type="text" id="name" name="name""></input></label><br/> <!-- Two separate fields? -->
    <label for="birthdate">Data di nascita: <br/><input type="date" id="birthdate" name="birthdate""></input></label><br/>
    <label for="gender">Gender: TODO /label><br/>
    <input type="submit" id="btn_register" name="btn_register" value="Invia"/>
</form><br/>